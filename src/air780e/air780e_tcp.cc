...
Air780ETcp::Air780ETcp(std::shared_ptr<AtUart> at_uart, int tcp_id) : at_uart_(at_uart), tcp_id_(tcp_id) {
    ...
    urc_callback_it_ = at_uart_->RegisterUrcCallback([this](const std::string& command, const std::vector<AtArgumentValue>& arguments) {
        if (command == "+TCPCONNECT" && arguments.size() == 2) {
            if (arguments[0].int_value == tcp_id_) {
                if (arguments[1].int_value == 0) {
                    ...
                    connected_ = false;
                    last_error_ = arguments[1].int_value;  // Store error code from TCPCONNECT response
        ...
        } else if (command == "+TCPSEND" && arguments.size() == 3) {
            if (arguments[0].int_value == tcp_id_) {
                if (arguments[1].int_value == 0) {
                    xEventGroupSetBits(event_group_handle_, AIR780E_TCP_SEND_COMPLETE);
                } else {
                    xEventGroupSetBits(event_group_handle_, AIR780E_TCP_SEND_FAILED);
                }
            }
        } else if (command == "+TCPURC" && arguments.size() >= 2) {
            if (arguments[1].int_value == tcp_id_) {
                if (arguments[0].string_value == "recv" && arguments.size() >= 4) {
                    ...
                    if (connected_) {
                        connected_ = false;
                        // instance_active_ 保持 true，需要发送 TCPCLS 清理
                        if (disconnect_callback_) {
                            disconnect_callback_();
                        }
                    }
                    ...
                    ESP_LOGE(TAG, "Unknown TCPURC command: %s", arguments[0].string_value.c_str());
                }
            }
        } else if (command == "+TCPSTATE" && arguments.size() > 5) {
            if (arguments[0].int_value == tcp_id_) {
                connected_ = arguments[5].int_value == 2;
                instance_active_ = true;
                xEventGroupSetBits(event_group_handle_, AIR780E_TCP_INITIALIZED);
            }
...
bool Air780ETcp::Connect(const std::string& host, int port) {
    ...
    at_uart_->SendCommand("AT+TCPCFG=\"close/mode\",1;+TCPCFG=\"viewmode\",1;+TCPCFG=\"sendinfo\",1;+TCPCFG=\"dataformat\",0,1");

    // 检查这个 id 是否已经连接
    std::string command = "AT+TCPSTATE=1," + std::to_string(tcp_id_);
    at_uart_->SendCommand(command);

    // 断开之前的连接（不触发回调事件）
    if (instance_active_) {
        at_uart_->SendCommand("AT+TCPCLS=" + std::to_string(tcp_id_));
        xEventGroupWaitBits(event_group_handle_, AIR780E_TCP_DISCONNECTED, pdTRUE, pdFALSE, TCP_CONNECT_TIMEOUT_MS / portTICK_PERIOD_MS);
        instance_active_ = false;
    }

    // 打开 TCP 连接
    command = "AT+TCPCONNECT=1," + std::to_string(tcp_id_) + ",\"TCP\",\"" + host + "\"," + std::to_string(port) + ",0,1";
    ...
    // 等待连接完成
    auto bits = xEventGroupWaitBits(event_group_handle_, AIR780E_TCP_CONNECTED | AIR780E_TCP_ERROR, pdTRUE, pdFALSE, TCP_CONNECT_TIMEOUT_MS / portTICK_PERIOD_MS);
...
void Air780ETcp::Disconnect() {
    ...
    if (at_uart_->SendCommand("AT+TCPCLS=" + std::to_string(tcp_id_))) {
        instance_active_ = false;
    }
...
int Air780ETcp::Send(const std::string& data) {
    ...
    while (total_sent < data.size()) {
        ...
        std::string command = "AT+TCPSEND=" + std::to_string(tcp_id_) + "," + std::to_string(chunk_size);
        ...
        auto bits = xEventGroupWaitBits(event_group_handle_, AIR780E_TCP_SEND_COMPLETE | AIR780E_TCP_SEND_FAILED, pdTRUE, pdFALSE, pdMS_TO_TICKS(TCP_CONNECT_TIMEOUT_MS));
        if (bits & AIR780E_TCP_SEND_FAILED) {
            ESP_LOGE(TAG, "Send failed, retry later");
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        } else if (!(bits & AIR780E_TCP_SEND_COMPLETE)) {
